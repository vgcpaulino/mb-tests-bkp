...
function create(protocols, imposters, logger, allowInjection) {
    function isFlagFalse(query, key) {
        return (
            !helpers.defined(query[key]) || query[key].toLowerCase() !== "false"
        );
    }

    function isFlagSet(query, key) {
        return (
            helpers.defined(query[key]) && query[key].toLowerCase() === "true"
        );
    }

    function validatePort(port, errors) {
        const portIsValid =
            !helpers.defined(port) ||
    ...
    function validateProtocol(protocol, errors) {
        ...
        if (!helpers.defined(protocol)) {
            errors.push(
                exceptions.ValidationError("'protocol' is a required field")
            );
        } else if (!Protocol) {
            errors.push(
                exceptions.ValidationError(
                    `the ${protocol} protocol is not yet supported`
                )
            );
        }
    }
    ...
    function respondWithValidationErrors(response, validationErrors) {
        logger.error(
            `error creating imposter: ${JSON.stringify(
    ...
    function requestDetails(request) {
        return `${helpers.socketName(request.socket)} => ${JSON.stringify(
            request.body
        )}`;
    }
    ...
    async function getAllJSON(queryOptions) {
        const allImposters = await imposters.all(),
            promises = allImposters.map((imposter) =>
                imposter.toJSON(queryOptions)
            );
    ...
    async function get(request, response) {
        const options = {
                ...
                list: !(
                    isFlagSet(request.query, "replayable") ||
                    isFlagSet(request.query, "removeProxies")
                ),
            },
        ...
        response.format({
            ...
            html: () =>
                response.render("imposters", { imposters: impostersJSON }),
        });
    }
        ...
        if (validation.isValid) {
            try {
                ...
                if (request.body.stubs && request.body.stubs.length > 0) {
                    request.body.stubs.forEach((s) => {
                        s.stubId = s.stubId || randomUUID();
                    });
                }
                const imposter = await protocols[protocol].createImposterFrom(
                    request.body
                );
    ...
    async function put(request, response) {
        const requestImposters = request.body.imposters || [],
            validationPromises = requestImposters.map((imposter) =>
                validate(imposter)
            );
        ...
        if (!isValid) {
            const validationErrors = validations.reduce(
                (accumulator, validation) =>
                    accumulator.concat(validation.errors),
                []
            );
        ...
        try {
            ...
            const promises = allImposters.map((imposter) =>
                    imposter.toJSON({ list: true })
                ),
