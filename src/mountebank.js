...
async function createApp(options) {
    ...
    const app = express(),
        ...
        protocols = protocolsModule.loadProtocols(
            options,
            baseURL,
            logger,
            isAllowedConnection,
        ...
        logfile = options.log.transports.file
            ? options.log.transports.file.path
            : false,
        logsController = logsControllerModule.create(logfile),
        configController = configControllerModule.create(
            thisPackage.version,
            options
        ),
    ...
    app.use(
        middleware.globals({ port: options.port, version: thisPackage.version })
    );
    ...
    app.delete(
        "/imposters/:id/savedProxyResponses",
        validateImposterExists,
        imposterController.resetProxies
    );
    app.delete(
        "/imposters/:id/savedRequests",
        validateImposterExists,
        imposterController.resetRequests
    );
    ...
    app.put(
        "/imposters/:id/stubs",
        validateImposterExists,
        imposterController.putStubs
    );
    app.put(
        "/imposters/:id/stubs/:stubIndex",
        validateImposterExists,
        imposterController.putStub
    );
    app.post(
        "/imposters/:id/stubs",
        validateImposterExists,
        imposterController.postStub
    );
    app.delete(
        "/imposters/:id/stubs/:stubIndex",
        validateImposterExists,
        imposterController.deleteStub
    );
    // eslint-disable-next-line no-warning-comments
    // TODO: @vgcpaulino changes;
    app.delete(
        "/imposters/:id/stubsById/:stubId",
        validateImposterExists,
        imposterController.deleteStubById
    );
    app.delete(
        "/imposters/:id/stubsById",
        validateImposterExists,
        imposterController.deleteStubsById
    );

    // Protocol implementation APIs
    app.post(
        "/imposters/:id/_requests",
        validateImposterExists,
        imposterController.postRequest
    );
    app.post(
        "/imposters/:id/_requests/:proxyResolutionKey",
        validateImposterExists,
        imposterController.postProxyResponse
    );
    ...
    if (options.allowInjection) {
        logger.warn(
            `Running with --allowInjection set. See ${baseURL}/docs/security for security info`
        );
    }
    ...
    return new Promise((resolve) => {
        const connections = {},
            server = app.listen(options.port, options.host, () => {
                logger.info(
                    `mountebank v${thisPackage.version} now taking orders - point your browser to ${baseURL}/ for help`
                );
                logger.debug(
        ...
        server.on("connection", (socket) => {
            ...
            socket.on("error", (error) => {
                logger.error(
                    `${name} transmission error X=> ${JSON.stringify(error)}`
                );
            });
